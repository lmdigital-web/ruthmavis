...
export const verifyPaystackPayment = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: reference }) => {
    ...
    /*
     * The payment has been confirmed by Paystack.
     *
     * The order was created before the customer was sent to Paystack.
     * Paystack metadata contains our order_id, so we can now securely
     * update that exact order using the server-side Supabase client.
     */
    const orderId = result.data?.metadata?.order_id;

    if (!orderId) {
      throw new Error('Payment succeeded but no order ID was found.');
    }
...
function CheckoutPage() {
  ...
  const selectedRate = useMemo(() => {
    if (!shipping.region || !shippingRates) {
      return null;
    }

    return shippingRates.find(
      r => r.region === shipping.region
    );
  ...
  const shippingAmount = useMemo(() => {
    ...
    if (
      selectedRate.free_shipping_threshold &&
      cartTotalPrice >= Number(selectedRate.free_shipping_threshold)
    ) {
      return 0;
    }
  ...
  /*
   * VAT is currently disabled for Ruth Mavis Accessories.
   * The business is not charging VAT, so the tax amount must remain 0.
   */
  const taxAmount = 0;

  const grandTotal = cartTotalPrice + shippingAmount;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="lg:hidden border-b border-gray-200 bg-gray-50 px-4 py-4">
        <button
          className="flex w-full items-center justify-between text-burgundy"
          onClick={() =>
            document
              .getElementById('mobile-summary')
      ...
      <div className="container mx-auto grid min-h-screen lg:grid-cols-[1fr_450px]">
        ...
        <div className="px-4 py-8 lg:px-12 lg:py-16">
          <div className="mb-8">
            <h1 className="text-2xl font-serif text-burgundy mb-4">
              Ruth Mavis Accessories
            </h1>
          ...
          >
            ...
            <div className="flex items-center justify-between pt-4">
              <Link
                to="/shop"
                className="text-burgundy flex items-center gap-1 text-sm"
              >
        ...
        <div className="hidden lg:block border-l border-gray-200 bg-gray-50 px-8 py-16">
          <div className="sticky top-16 space-y-6">
            ...
            {selectedRate?.free_shipping_threshold &&
              cartTotalPrice <
                Number(selectedRate.free_shipping_threshold) && (
